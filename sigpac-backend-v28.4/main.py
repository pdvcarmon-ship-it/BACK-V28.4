...
async def _datos_siar_periodo(estacion_id: str, fecha_ini: str, fecha_fin: str) -> list:
    ...
    try:
        params = {
            ...
            "Variables": "TempMedia,TempMax,TempMin,Precipitacion,EtPMon",
        }
        async with httpx.AsyncClient(timeout=30) as client:
...
async def _clima_desde_siar(lat: float, lon: float) -> dict:
    ...
    # Una sola peticion: todo el año agricola hasta hoy
    datos = await _datos_siar_periodo(
        estacion["id"],
    ...
    # El ultimo registro = dia mas reciente
    obs = datos[-1]
    ...
    eto_dia    = num(obs.get("EtPMon"))  # Penman-Monteith del SIAR
    ...
    # Acumular todos los registros del año agricola
    precip_acum = sum(num(d.get("Precipitacion"), 0.0) or 0 for d in datos)
    eto_acum    = sum(num(d.get("EtPMon"),        0.0) or 0 for d in datos)
