...
async def _cargar_estaciones_siar() -> list:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            r = await client.get(f"{SIAR_BASE}/API/V1/Info/ESTACIONES?token={token_enc}")
            r.raise_for_status()
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            url = (f"{SIAR_BASE}/API/V1/Datos/Diarios/ESTACION"
                   f"?token={token_enc}&Estacion={estacion_id}"
                   f"&FechaInicio={fecha_ini}&FechaFin={fecha_fin}")
            r = await client.get(url)
            if r.status_code == 400:
                logger.warning(f"SIAR 400 para {estacion_id}: {r.text[:200]}")
                return []
            r.raise_for_status()
            data = r.json()
