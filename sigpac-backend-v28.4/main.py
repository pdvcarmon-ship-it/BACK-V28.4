    ...
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            ...
            if isinstance(token, str) and len(token) > 3:
                ...
                logger.info(f"SIAR: token obtenido OK (...{token[-8:]})")
                return _siar_token
            else:
                logger.error(f"SIAR: respuesta token inesperada: {token}")
    except Exception as ex:
        logger.error(f"Error obteniendo token SIAR: {ex}")
    return ""
...
async def _cargar_estaciones_siar() -> list:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            r = await client.get(f"{SIAR_BASE}/API/V1/Info/ESTACIONES?token={token}")
            r.raise_for_status()
            estaciones_raw = r.json().get("datos", [])
        resultado = []
        for e in estaciones_raw:
            try:
                lat = _parse_dms(str(e.get("Latitud", "")))
                lon = _parse_dms(str(e.get("Longitud", "")))
...
async def _datos_siar_periodo(estacion_id: str, fecha_ini: str, fecha_fin: str) -> list:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            url = (f"{SIAR_BASE}/API/V1/Datos/Diarios/ESTACION"
                   f"?token={token}&Estacion={estacion_id}"
                   f"&FechaInicio={fecha_ini}&FechaFin={fecha_fin}")
...
async def _clima_desde_siar(lat: float, lon: float) -> dict:
    ...
    datos_recientes = await _datos_siar_periodo(estacion["id"], ayer.strftime("%Y-%m-%d"), hoy.strftime("%Y-%m-%d"))
    datos_agricola  = await _datos_siar_periodo(estacion["id"], inicio_agricola.strftime("%Y-%m-%d"), ayer.strftime("%Y-%m-%d"))
    ...
    obs = datos_recientes[-1] if datos_recientes else {}
    temp_max   = num(obs.get("Tmax"))
    temp_min   = num(obs.get("Tmin"))
    temp_med   = num(obs.get("Tmed"))
    precip_dia = num(obs.get("Precip"), 0.0)
    eto_dia    = num(obs.get("Eto"))
    ...
    precip_acum = sum(num(d.get("Precip"), 0.0) or 0 for d in datos_agricola)
    eto_acum    = sum(num(d.get("Eto"),    0.0) or 0 for d in datos_agricola)
