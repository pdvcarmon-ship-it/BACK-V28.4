...
_fito_productos: list = []
_fito_cargado: bool = False
FITO_JSON_PATH = os.path.join(os.path.dirname(__file__), "ProductosAutorizados_2026_07_05.json")


def _cargar_fito():
    global _fito_productos, _fito_cargado
    if _fito_cargado:
        return
    try:
        with open(FITO_JSON_PATH, "r", encoding="utf-8-sig") as f:
            data = json.load(f)
        # Solo productos vigentes, campos esenciales
        _fito_productos = [
            {
                "id": p["IdProducto"],
            ...
            for p in data
            if p.get("Estado") == "Vigente" and p.get("Nombre")
        ]
        _fito_cargado = True
        logger.info(f"Fitosanitarios: {len(_fito_productos)} productos vigentes cargados")
    except Exception as ex:
...
@app.get("/fito/buscar")
async def fito_buscar(q: str = Query(..., min_length=2, description="Nombre o número de registro")):
    """Busca productos fitosanitarios por nombre o número de registro."""
    _cargar_fito()
    q_low = q.lower().strip()
    resultados = [
        p for p in _fito_productos
...
async def fito_detalle(id_producto: int):
    """Devuelve los datos básicos de un producto y la URL de su ficha PDF oficial."""
    _cargar_fito()
    producto = next((p for p in _fito_productos if p["id"] == id_producto), None)
    if not producto:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    return {
        **producto,
        "ficha_pdf_url": f"https://servicio.mapa.gob.es/regfiweb/Productos/FichaPDF/{id_producto}",
        "ficha_web_url": f"https://servicio.mapa.gob.es/regfiweb/Productos/DetalleProducto/{id_producto}",
    }
